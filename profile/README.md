...
* **`trace-cloud-backend`**: Azure IoT Hub routing, MQTT decompression, and PostgreSQL Flexible Server data persistence.
* **`trace-ml-engine`**: Azure Machine Learning environment running the unsupervised Isolation Forest algorithm and automated continuous learning pipelines.
* **`trace-rag-diagnostics`**: Azure OpenAI integration handling diagnostic routing, hardware asset registry cross-referencing, and feedback loop ingestion.
* **`trace-admin-dashboard`**: Next.js observability frontend for spatial heatmaps, the maintenance queue, and one-click remediation dispatch.
...
**Commit Conventions**
All commit messages must follow the Conventional Commits standard to automate changelogs and maintain a clean history:
